...
# Define paths
input_folder = "/home/ivo/Downloads/ftx/withdrawals"
output_file = "withdrawals_output.tsv"

# Regular expressions to match the required patterns
withdrawal_pattern = re.compile(r"Your withdrawal of ([\d.]+) (\w+) to the address ending in \w+ has been sent. Please allow some time for the transfer to arrive.")
...
with open(output_file, "w") as out_file:
    ...
    out_file.write("DateTime\tAmount\tCoin\n")
    ...
    for filename in os.listdir(input_folder):
        if filename.endswith(".eml"):
            ...
            with open(file_path, "r") as eml_file:
                ...
                # Search for the withdrawal pattern in the email body
                withdrawal_match = withdrawal_pattern.search(body)

                # Search for the date-time in the headers
                headers = msg.as_string()
                header_match = header_pattern.search(headers)

                if withdrawal_match and header_match:
                    amount, coin = withdrawal_match.groups()
                    raw_date_time = header_match.group(1)
                    formatted_date_time = convert_date_time(raw_date_time)

                    # Write the extracted data to the output file
                    out_file.write(f"{formatted_date_time}\t{amount}\t{coin}\n")
